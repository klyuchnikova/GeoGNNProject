...
class Flashback(nn.Module):
    ...
    def __init__(self, input_size, user_count, hidden_size, f_t, f_s, rnn_factory, lambda_loc, lambda_user, use_weight,
        ...
        if friend_graph is not None:
            friend_i = identity(friend_graph.shape[0], format='coo')
            self.friend_graph = sparse_matrix_to_tensor(
                calculate_random_walk_matrix((friend_graph * self.lambda_user + friend_i).astype(np.float32)))
        else:
    ...
    def forward(self, x, t, t_slot, s, y_t, y_t_slot, y_s, h, active_user):
        ...
        if self.use_graph_user:
            ...
            if self.friend_graph is None:
                raise ValueError('use_graph_user=True requires a friend graph file.')
