"""Install the public Foursquare Tokyo check-in dataset for Kaggle runs.

The script first looks for an attached Kaggle input containing the standard
``dataset_TSMC2014_TKY.txt`` file. If not found, it downloads the public Kaggle
archive ``chetanism/foursquare-nyc-and-tokyo-checkin-dataset`` and extracts the
Tokyo check-ins into ``data/input/foursquare_tokyo_checkins.txt``.
"""
...
TOKYO_NAMES = [
    ...
    "dataset_tsmc2014_tky.csv",
    "foursquare_tokyo_checkins.txt",
    "foursquare_tokyo_checkins.csv",
]
...
def extract_tokyo(archive: Path, work_dir: Path) -> Path:
    with zipfile.ZipFile(archive) as zf:
        members = zf.namelist()
        tokyo_member = None
        for member in members:
            lower = Path(member).name.lower()
            if lower in {name.lower() for name in TOKYO_NAMES} or ("tokyo" in lower and lower.endswith((".txt", ".csv"))):
                tokyo_member = member
                break
        if tokyo_member is None:
            raise FileNotFoundError(f"Tokyo check-in file not found inside {archive}; members={members[:20]}")
        work_dir.mkdir(parents=True, exist_ok=True)
        zf.extract(tokyo_member, work_dir)
        return work_dir / tokyo_member


def install(source: Path | None, output: Path) -> Path:
    output.mkdir(parents=True, exist_ok=True)
    target = output / "foursquare_tokyo_checkins.txt"
    ...
    if source is not None:
        ...
        if source.is_file() and zipfile.is_zipfile(source):
            found = extract_tokyo(source, output / ".foursquare_extract")
        elif source.is_dir():
            found = search_file([source])
            if found is None:
                raise FileNotFoundError(f"No Foursquare Tokyo check-in file found under {source}")
        else:
            found = source
    else:
        ...
        if found is None:
            ...
            found = extract_tokyo(archive, output / ".foursquare_extract")
    ...
    if sample.shape[1] < 8:
        sample = pd.read_csv(target, sep=",", header=None, nrows=3, engine="python")
    if sample.shape[1] < 5:
        raise ValueError(f"Foursquare Tokyo file looks too narrow: {sample.shape[1]} columns")
    print(f"Installed Foursquare Tokyo check-ins: {target}")
...
def main() -> None:
    ...
    parser.add_argument("--source", type=Path, default=None, help="optional archive, extracted directory, or Tokyo check-in file")
